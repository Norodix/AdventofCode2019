...
char tiles_copy[FIELDSIZE][FIELDSIZE];
WINDOW *w = NULL;
...
int main(int argc, char** argv) {
    ...
    int empty_found = 1;
    int minutes = 0;
    memcpy(tiles_copy, tiles, sizeof(tiles));
    while(empty_found) {
        minutes++;
        empty_found = 0;
        for (int r = 0; r < FIELDSIZE; r++) {
            for (int c = 0; c < FIELDSIZE; c++) {
                // Spread oxygen to neighbors
                if (tiles[r][c] == OXYGEN) {
                    char* t;
                    t = &tiles_copy[r+1][c];
                    if (*t == WALKED) *t = OXYGEN;
                    t = &tiles_copy[r-1][c];
                    if (*t == WALKED) *t = OXYGEN;
                    t = &tiles_copy[r][c+1];
                    if (*t == WALKED) *t = OXYGEN;
        ...
        memcpy(tiles, tiles_copy, sizeof(tiles));
        for (int r = 0; r < FIELDSIZE; r++) {
            for (int c = 0; c < FIELDSIZE; c++) {
                if (tiles[r][c] == WALKED) {
                    empty_found = 1;
    ...
    printf("It took %d minutes to fill the whole area with oxygen\n", minutes);
    return 0;
}
